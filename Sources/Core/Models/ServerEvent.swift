...
@Codable @CodedAt("type") public enum ServerEvent: Sendable {
    ...
    case inputAudioBufferTimeoutTriggered(
        eventId: String,
        itemId: String,
        audioStartMs: Int,
        audioEndMs: Int
    ...
    ///
    /// - Parameter eventId: The unique ID of the server event.
    /// - Parameter responseId: The ID of the Response to which the output audio belonged.
...
extension ServerEvent: Identifiable {
    public var id: String {
        ...
        case .sessionCreated(let id, _): id
        case .sessionUpdated(let id, _): id
        case .conversationItemAdded(let id, _, _): id
        case .conversationItemCreated(let id, _, _): id
        case .conversationItemDone(let id, _, _): id
        case .conversationItemRetrieved(let id, _): id
        case .conversationItemInputAudioTranscriptionCompleted(
            let id,
            _,
            _,
        ...
        case .conversationItemInputAudioTranscriptionDelta(let id, _, _, _, _):
            id
        case .conversationItemInputAudioTranscriptionSegment(
            let id,
            _,
        ...
        case .conversationItemInputAudioTranscriptionFailed(let id, _, _, _): id
        case .conversationItemTruncated(let id, _, _, _): id
        case .conversationItemDeleted(let id, _): id
        case .inputAudioBufferCommitted(let id, _, _): id
        case .inputAudioBufferCleared(let id): id
        case .inputAudioBufferSpeechStarted(let id, _, _): id
        case .inputAudioBufferSpeechStopped(let id, _, _): id
        case .inputAudioBufferTimeoutTriggered(let id, _, _, _): id
        case .outputAudioBufferStarted(let id, _): id
        case .outputAudioBufferStopped(let id, _): id
        case .outputAudioBufferCleared(let id, _): id
        case .responseCreated(let id, _): id
        case .responseDone(let id, _): id
        case .responseOutputItemAdded(let id, _, _, _): id
        case .responseOutputItemDone(let id, _, _, _): id
        case .responseContentPartAdded(let id, _, _, _, _, _): id
        case .responseContentPartDone(let id, _, _, _, _, _): id
        case .responseTextDelta(let id, _, _, _, _, _): id
        case .responseTextDone(let id, _, _, _, _, _): id
        case .responseAudioTranscriptDelta(let id, _, _, _, _, _): id
        case .responseAudioTranscriptDone(let id, _, _, _, _, _): id
        case .responseOutputAudioDelta(let id, _, _, _, _, _): id
        case .responseOutputAudioDone(let id, _, _, _, _): id
        case .responseFunctionCallArgumentsDelta(let id, _, _, _, _, _): id
        case .responseFunctionCallArgumentsDone(let id, _, _, _, _, _): id
        case .responseMCPCallArgumentsDelta(let id, _, _, _, _, _): id
        case .responseMCPCallArgumentsDone(let id, _, _, _, _): id
        case .mcpListToolsInProgress(let id, _): id
        case .mcpListToolsCompleted(let id, _): id
        case .mcpListToolsFailed(let id, _): id
        case .responseMCPCallInProgress(let id, _, _): id
        case .responseMCPCallCompleted(let id, _, _): id
        case .responseMCPCallFailed(let id, _, _): id
        case .rateLimitsUpdated(let id, _): id
        }
    }
}
